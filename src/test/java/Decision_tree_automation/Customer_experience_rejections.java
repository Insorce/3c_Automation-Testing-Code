...
public class Customer_experience_rejections 
{

	@Test
	public static void Customer_experience_rejections() throws Exception
	{
		...
		WebElement reductions=rework.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='dvLeversSection']/descendant::a[@href='/Operations/TimeUndesiredOutcomes']")));
		...
		for( int j=1; j<=view.size(); j++)
		{
			...
			//click on view 
			WebElement click_view=driver.findElement(By.xpath("//tr[@class='info']["+j+"]//td[5]//a"));
		...
		objSelect.selectByVisibleText("Effort Saved");
		Thread.sleep(4000);
		...
		WebElement unit_mapping=driver.findElement(By.xpath("//a[text()='Refer unit mapping']"));
		JavascriptExecutor click_download_button= (JavascriptExecutor) driver;
		click_download_button.executeScript("arguments[0].click();", unit_mapping);
...
//
//			
//			List<WebElement> download_as_csv=driver.findElements(By.xpath("//a[text()=' Download as CSV']"));
//			for (int c = 1; c <= download_as_csv.size(); c++)
//			{
//				WebElement click_tgf=driver.findElement(By.xpath("(//a[text()=' Download as CSV'])["+c+"]"));
//
//				JavascriptExecutor pdf_click = (JavascriptExecutor) driver;
//				pdf_click.executeScript("arguments[0].click();", click_tgf);
//			}
